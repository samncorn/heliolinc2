...
py::array observer_vel(double detmjd, double lon, double obscos, double obssine, py::array_t<EarthState> earthin)
{
  ...
  std::cout << earthnow.x << ", " << earthnow.y << ", " << earthnow.z << std::endl;
  outvec={};
  outvec.push_back(earthnow.x);
  outvec.push_back(earthnow.y);
  outvec.push_back(earthnow.z);
  ...
  // fileName.setf(std::ios::scientific);
  // fileName.open( "timing.txt", std::ios::app );
  // gettimeofday(&t2, NULL);
  // tDiff = (t2.tv_sec - t1.tv_sec) + (t2.tv_usec - t1.tv_usec) / 1e6;
  // fileName << tDiff << std::endl; 
  // fileName.close();
  // //================================
...
// py::array observer_vel_timed(py::array_t<double> detmjds, double lon, double obscos, double obssine, py::array_t<EarthState> earthin)
// {
//   float t0, tf;
//   // t0 = get_time();
...
py::array observer_vel(py::array_t<double> detmjds, double lon, double obscos, double obssine, py::array_t<EarthState> earthin)
{
  int n = detmjds.size(); 
  // std::vector<double> vec_mjd=ndarray_to_vec(detmjds);
  std::vector <EarthState> earthpos = ndarray_to_vec(earthin);
  int polyorder=5;
  std::vector <double> posmjd;
  std::vector <point3d> planetpos;
  ...
  point3d earthnow = point3d(0,0,0);
  point3d earthvel = point3d(0,0,0);
  std::vector <double> outvec(6);
  std::vector <std::vector<double>> out_vec_all; //(n, 6);
  double detmjd;
  ...
  for(j=0; j<n; j++) {
    ...
    // detmjd = mjd_ref[j];
    detmjd = detmjds.at(j);
    // std::cout << j << ", " << std::setprecision (15) << detmjd << std::endl;
    observer_baryvel01(detmjd, polyorder, lon, obscos, obssine, posmjd, planetpos, planetvel, earthnow, earthvel);
    // std::cout << earthnow.x << ", " << earthnow.y << ", " << earthnow.z << std::endl;
    // outvec.push_back(earthnow.x);
    // outvec.push_back(earthnow.y);
    ...
    outvec[4] = earthvel.y;
    outvec[5] = earthvel.z;
    out_vec_all.push_back( outvec );
  }
 
  // auto py_outvec = py::array_t<double>(outvec.size());
  // auto py_outvec = py::array_t<double>(out_vec_all.size());
  py::array_t<double, py::array::c_style> py_outvec( {n, 6} );
  auto data_ref = py_outvec.mutable_unchecked();
  // Place vector data into numpy array
  for (i = 0; i < n; i++) {
    for (j = 0; j < 6; j++) {
      data_ref(i, j) = out_vec_all[i][j];
      // auto data_in = out_vec_all[i][j];
      // auto &data_out = data_ref.data(i, j);
      // data_out = data_in;
    }
  }
...
std::tuple<py::array, py::array, py::array> makeTracklets(
  ...
  // cout << "generating tracklets ...\n";
  py::print( "generating tracklets ..." );
...
PYBIND11_MODULE(heliohypy, m) {
    ...
    py::class_<hlimage>(m, "hlimage")
      ...
      .def_readonly( "MJD", &hlimage::MJD )
      .def_readonly( "RA", &hlimage::RA )
      .def_readonly( "Dec", &hlimage::Dec )
      .def_readonly( "obscode", &hlimage::obscode )
      .def_readonly( "X", &hlimage::X )
      .def_readonly( "Y", &hlimage::Y )
      .def_readonly( "Z", &hlimage::Z )
      .def_readonly( "VX", &hlimage::VX )
      .def_readonly( "VY", &hlimage::VY )
      .def_readonly( "VZ", &hlimage::VZ )
      .def_readonly( "startind", &hlimage::startind )
      .def_readonly( "endind", &hlimage::endind )
      ;
    ...
    m.def("observer_vel", py::overload_cast<double, double, double, double, py::array_t<EarthState>> (&observer_vel), "calculate position of an observer on Earth");
    m.def("observer_vel", py::overload_cast<py::array_t<double>, double, double, double, py::array_t<EarthState>> (&observer_vel), "calculate position of an observer on Earth");
    // m.def("observer_vel_vectorized", &observer_vel_vectorized, "calculate position of an observer on Earth");

    // m.def("makeTracklets", &makeTracklets, "Make tracklets from set of detections.");

    m.def("makeTracklets", &makeTracklets, py::call_guard<py::scoped_ostream_redirect, py::scoped_estream_redirect>(), "Make tracklets from set of detections.");
    // m.def("makeTracklets", []() {
