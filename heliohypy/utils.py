...
# import heliolinc
from heliohypy import heliohypy
...
def load_opsim( file_path, columns=None, mjd_start=None, mjd_stop=None ):
    """ load images from operations simulator database

    automatically loads image start time, additional columns may be requested by the columns param
    """
    if columns is not None:
        columns_string = ' observationStartMJD, '+', '.join( columns )
    else:
        columns_string = ' observationStartMJD'
    select_string = 'SELECT'+columns_string+' FROM observations'
    order_string  = ' ORDER BY observationStartMJD'

    query_string = select_string+order_string
    
    if mjd_start is not None or mjd_stop is not None:
        where_string = ' WHERE'

        if mjd_start is not None:
            where_string += f' observationStartMJD >= {mjd_start}'
        if mjd_stop is not None:
            where_string += f' observationStartMJD <= {mjd_stop}'

        query_string += where_string

    conn = sql.connect( file_path )
    opsim = pd.read_sql_query( query_string, conn )
    return opsim

def load_obscodes( file_path ):
    obscodes = {}
    with open( file_path, '+r' ) as obscodes_files:
        for line in obscodes_files.readlines()[1:]:
            obscodes[ line[0:3] ] = {
                'obscode' : line[0:3],
                'Long' : float(line[4:13]),
                'pcos' : float(line[13:21]),
                'psin' : float(line[24:32]),
            }
    return obscodes

""" Generate heliolinc formatted images for a single observer

params
======
MJD         ... image times
RA, Dec     ... angular measurements
observer    ... dictionary (tuple?) with observatory info from mpc obscode file
earth_ephem ... array of earth states
"""
def format_images( MJD, RA, Dec, observer, earth_ephem ):
    n = len(MJD)
    images = np.empty((n,),dtype=hlimage)

    obs_states = heliohypy.observer_vel(
        MJD,
        observer['Long'],
        observer['pcos'],
        observer['psin'],
    ...
    # load the stuff we know
    images['MJD']        = MJD
    images['RA']         = RA
    images['Dec']        = Dec
    images['obscode']    = observer['obscode']
    images['X']          = obs_states[:,0]
    images['Y']          = obs_states[:,1]
    images['Z']          = obs_states[:,2]
    images['VX']         = obs_states[:,3]
    images['VY']         = obs_states[:,4]
    images['VZ']         = obs_states[:,5]
...
def format_detections( MJD, RA, Dec, mag, idstring, image_idx, band, obscode, ):
    n = len( MJD )

    dets = np.empty((n,),dtype=hldet)
    dets['MJD']         = MJD
    dets['RA']          = RA
    dets['Dec']         = Dec
    dets['mag']         = mag
    dets['idstring']    = idstring
    dets['image']       = image_idx
    dets['band']        = band
    dets['obscode']     = obscode
